...
# Targeted Testing
t_UAP = art.attacks.evasion.TargetedUniversalPerturbation(classifier=classifier, attacker='fgsm',eps=0.005, norm=np.inf)

input_array = input_batch.numpy()
# targeted
adv_im = t_UAP.generate(x=input_array, y=targetLabel)
# un-targeted
# adv_im = t_UAP.generate(x=input_array)
...
t_UAP_pert = (torch.from_numpy(pert_im) - input_tensor)
plt.imshow(tf_mod(t_UAP_pert))
plt.title(str_label_pert)
plt.show()
