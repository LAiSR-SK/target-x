# Contains functions called for testing immunity of Target-X, Target-FGSM, and Target-UAP.
# Testing function for all methods, takes the original network and 4 networks finetuned on each method as the input.
...
# Testing function for the Target-X algorithm, takes an original network, 
# a network finetuned on images generated by Target-X, an epsilon value, and a file name as the inputs.
...
# Testing function for the Target-FGSM algorithm (implemented by ART), takes an original network, 
# a network finetuned on images generated by Target-FGSM, an epsilon value, and a file name as the inputs.
...
# Testing function for the Target-UAP algorithm (implemented by ART), takes an original network, 
# a network finetuned on images generated by Target-UAP, an epsilon value, and a file name as the inputs.
# This algorithms implementation results in CUDA errors for immunity testing, and should be looked into further by the ART developers at.
def targetUAP_Immunity_Testing(orig_net, targetuap_net, eps, csvfilename):
    ...
    for filename in glob.glob('D:/imageNet/ImagenetDataset/ILSVRC2012/ILSVRC/Data/CLS-LOC/val/*.JPEG'):
        ...
        uap_rows = []
        uap_rows.append([filename[47:75], str_label_orig_correct, str_label_orig_orig, str_label_orig_pert, str_label_result])

        with open(tuap_csv, 'a', newline='') as csvfile:
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(uap_rows)
        counter = counter + 1
